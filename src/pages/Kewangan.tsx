...
import { ROLE_LABELS, CONTACT_INFO } from "@/lib/constants";
...
// Using constants from lib/constants.ts for better maintainability
          ...
          <div className="space-y-4 pt-4">
            <div className="glass-card rounded-2xl p-4 bg-green-500/5 border border-green-500/20">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="font-semibold text-foreground">{CONTACT_INFO.BANK_NAME}</p>
              ...
              <div className="space-y-3">
                <div className="bg-background/50 rounded-xl p-3">
                  ...
                  <div className="flex items-center justify-between">
                    <p className="font-mono font-bold text-lg text-foreground">{CONTACT_INFO.BANK_ACCOUNT_NO}</p>
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      className="h-8 rounded-lg"
                      onClick={() => copyToClipboard(CONTACT_INFO.BANK_ACCOUNT_NO)}
                    >
                      {copied ? (
                ...
                <div className="bg-background/50 rounded-xl p-3">
                  ...
                  <p className="font-medium text-sm text-foreground">{CONTACT_INFO.BANK_ACCOUNT_NAME}</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-green-600 dark:text-green-500">
                    WhatsApp bukti pembayaran kepada bendahari
                  </p>
                </div>
                <Button
                  asChild
                  variant="outline"
                  size="sm"
                  className="rounded-xl gap-2 bg-green-500/10 border-green-500/30 hover:bg-green-500/20"
                >
                  <a
                    href={`https://wa.me/${CONTACT_INFO.TREASURER_WHATSAPP.replace(/[^0-9]/g, '')}?text=Salam%2C+saya+sudah+membuat+payment+yuran+bulanan+RM+${totalSelectedAmount.toFixed(2)}+untuk+${selectedMonths.map(m => BULAN_NAMES[m - 1]).join('%2C+')}+${currentYear}.+No+ahli:+${userProfile?.nama_penuh || 'Ahli'}.+Terima+kasih.`}
                    target="_blank"
