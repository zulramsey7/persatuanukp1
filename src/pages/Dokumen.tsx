...
export default function Dokumen() {
  ...
  const getDocSignedUrl = async (doc: Dokumen) => {
    if (signedUrls[doc.id]) return signedUrls[doc.id];

    try {
      // Extract file name from URL or use it directly if it's a path
  ...
  const handleView = async (doc: Dokumen) => {
  ...
  const handleDownload = async (doc: Dokumen) => {
    const url = await getDocSignedUrl(doc);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/30">
      ...
      <main className={`min-h-screen transition-all duration-300 ${
        isMobile ? 'px-4 pb-24 pt-4' : sidebarCollapsed ? 'ml-20 p-8' : 'ml-[280px] p-8'
      }`}>
        {/* Header */}
        <motion.div
        ...
        >
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center shadow-lg">
                  <FolderOpen className="w-6 h-6 md:w-7 md:h-7 text-primary-foreground" />
                </div>
                <div>
