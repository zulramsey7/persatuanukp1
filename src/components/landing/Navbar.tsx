        ...
        <div className="container px-4">
          <div className={cn("flex items-center justify-between rounded-full px-6 py-3 transition-all duration-300", isScrolled ? "floating-card" : "bg-transparent")}>
            ...
            <nav className="hidden md:flex items-center gap-1">
              {navLinks.map(link => <a key={link.label} href={link.href} className="relative px-4 py-2 text-sm font-semibold text-foreground/70 hover:text-foreground transition-all duration-300 hover:bg-primary/5 rounded-lg group">
                  <span className="relative z-10">{link.label}</span>
                  <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-gradient-to-r from-primary to-primary/60 group-hover:w-full transition-all duration-300 rounded-full" />
                </a>)}
            </nav>
        ...
        y: -20
            ...
            <nav className="container px-4 py-8">
              ...
              delay: index * 0.1
                    ...
                    <a href={link.href} className="text-2xl font-bold text-foreground/80 hover:text-primary transition-all duration-300 hover:pl-4 hover:bg-primary/5 rounded-lg px-4 py-3" onClick={() => setIsMobileMenuOpen(false)}>
                      {link.label}
                    </a>
                  </motion.li>)}
              </ul>
