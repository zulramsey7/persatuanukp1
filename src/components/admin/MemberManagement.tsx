...
const MemberManagement = () => {
  ...
  const { isPengerusi, isSetiausaha, isBendahari, isNaibPengerusi, user } = useAuth();
  ...
  const handleStatusChange = async (memberId: string, newStatus: StatusAhli) => {
    try {
      ...
      const oldStatus = members.find(m => m.id === memberId)?.status_ahli;
                ...
                >
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2 flex-wrap">
                      {/* Yuran Management - For Pengerusi, Setiausaha, and Bendahari */}
                      {(isPengerusi || isSetiausaha || isBendahari) && (
                        <Button
                          size="sm"
                          variant="outline"
                      ...
                      {/* Role Management - Pengerusi, Setiausaha & Naib Pengerusi */}
                      {(isPengerusi || isSetiausaha || isNaibPengerusi) && member.id !== user?.id && (
                        <Button
                          size="sm"
                          variant="outline"
