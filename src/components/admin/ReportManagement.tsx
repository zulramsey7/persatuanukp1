...
export default function ReportManagement() {
  ...
  // Neutralize potential CSV injection by prefixing dangerous characters with a single quote
  const csvSafe = (val: string | number | null | undefined): string => {
    if (val === null || val === undefined) return "";
    const str = String(val);
    // If it starts with a character that can trigger a formula, prefix with a quote
    if (/^[=\+\-@]/.test(str)) {
      return `'${str}`;
    }
    return str;
  ...
  const generateMemberReport = async () => {
    try {
      ...
      const csvContent = [
        headers.join(","),
        ...data.map(item => 
          `"${csvSafe(item.nama_penuh)}","${csvSafe(item.no_rumah)}","${csvSafe(item.no_telefon || '-')}","${csvSafe(item.status_ahli)}","${csvSafe(format(new Date(item.created_at), "dd/MM/yyyy"))}"`
        )
      ].join("\n");
  ...
  const generateFinanceReport = async () => {
    try {
      ...
      const incomeRows = (incomes || []).map(item => 
        `"Pendapatan","Yuran Bulan ${csvSafe(item.bulan)}/${csvSafe(item.tahun)}","${csvSafe(item.jumlah)}","${item.tarikh_bayar ? csvSafe(format(new Date(item.tarikh_bayar), "dd/MM/yyyy")) : '-'}","${csvSafe(item.status)}"`
      );
      const expenseRows = (expenses || []).map(item => 
        `"Perbelanjaan","${csvSafe(item.tajuk_belanja)}","-${csvSafe(item.jumlah)}","${csvSafe(format(new Date(item.tarikh), "dd/MM/yyyy"))}","${csvSafe(item.kategori)}"`
      );
  ...
  const generateActivityReport = async () => {
     try {
      ...
      const csvContent = [
        headers.join(","),
        ...data.map(item => 
          `"${csvSafe(item.tajuk)}","${csvSafe(format(new Date(item.tarikh_mula), "dd/MM/yyyy"))}","${csvSafe(item.lokasi)}","${csvSafe(item.deskripsi || '-')}"`
        )
      ].join("\n");
