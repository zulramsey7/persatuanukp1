...
export function RecentTables({
  ...
  const filteredIncomes = useMemo(() => {
    return incomes.filter((income) => {
      ...
      const incomDate = income.tarikh_bayar ? new Date(income.tarikh_bayar) : null;
      const matchesDateRange =
        (!incomeStartDate || !incomDate || incomDate >= new Date(incomeStartDate)) &&
        (!incomeEndDate || !incomDate || incomDate <= new Date(incomeEndDate));
  ...
  const handleExportIncomes = () => {
    ...
    const data = filteredIncomes.map(income => ({
      ...
      "Tarikh": income.tarikh_bayar ? format(new Date(income.tarikh_bayar), "d MMM yyyy", { locale: ms }) : "-",
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {income.tarikh_bayar ? format(new Date(income.tarikh_bayar), "d MMM yyyy", { locale: ms }) : "-"}
                  </p>
                </div>
                  ...
                  {paginatedIncomes.map((income) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {income.tarikh_bayar ? format(new Date(income.tarikh_bayar), "d MMMM yyyy", { locale: ms }) : "-"}
                        </p>
                      </div>
                      <div className="text-right">
