...
export function CommunityInfoWidget() {
  ...
  const fetchCommunityStats = async () => {
    try {
      const [
        ...
        supabase.from("dana_masuk").select("jumlah"),
        supabase.from("aktiviti").select("*", { count: "exact", head: true }).eq("status", "aktif").gte("tarikh_mula", new Date().toISOString())
      ]);
      ...
      const totalFunds = totalIncome + totalDanaMasuk;
      ...
      setStats({
        ...
        totalDanaMasuk,
        totalFunds,
        upcomingEvents: aktivitiRes.count || 0
      });
    } catch (error) {
  ...
  const infoItems = [
    ...
    },
    {
      ...
      value: `RM ${stats.totalFunds.toLocaleString()}`,
      subValue: `Yuran + Sumbangan`,
