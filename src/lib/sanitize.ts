...
/**
 * Sanitize name input to prevent XSS attacks while allowing spaces
 * Specifically designed for names that need to preserve spaces
 */
export const sanitizeName = (input: string | null | undefined): string => {
  if (!input) return '';

  // Convert to string if needed, preserve all spaces for typing
  ...
  // Don't trim - preserve all spaces for real-time typing
  return str;
};
