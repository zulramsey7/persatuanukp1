...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchRoles = async (userId: string) => {
    try {
      ...
      const cachedRoles = localStorage.getItem(`roles_${userId}`);
      if (cachedRoles) {
      ...
      if (data) {
        setRoles(data);
        // Save to cache for offline access
        localStorage.setItem(`roles_${userId}`, JSON.stringify(data));
      }
    } catch (error) {
  ...
  // Derived permissions - Setiausaha now has same permissions as Pengerusi
  const canDeleteData = isPengerusi || isSetiausaha;
